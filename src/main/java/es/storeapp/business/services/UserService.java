...
public class UserService {
    ...
    public User login(String email, String clearPassword) throws AuthenticationException {
        ...
        User user = userRepository.findByEmailAndPassword(email, BCrypt.hashpw(clearPassword,BCrypt.gensalt()));
        if (user == null) {
            throw exceptionGenerationUtils.toAuthenticationException(Constants.AUTH_INVALID_PASSWORD_MESSAGE, email);// No debría ser invalid password porque da demasiadas pistas (?)
        }
        return user;
    }
    ...
    public User update(Long id, String name, String email, String address, String image, byte[] imageContents)
        ...
        if (image != null && image.trim().length() > 0 && imageContents != null) {
            ...
            } catch (Exception ex) {// No pone el tipo de excepción
                logger.error(ex.getMessage(), ex);
            }
    ...
    public User changePassword(Long id, String oldPassword, String password)
        ...
        if (user == null) {
            throw exceptionGenerationUtils.toAuthenticationException(
                    Constants.AUTH_INVALID_USER_MESSAGE, id.toString());// Muestra el id del user?
        }
